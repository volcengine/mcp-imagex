...
def create_mcp_server():
    ...
    def guide():
        ...
        1. For tools that rely on service_id and domain, prioritize obtaining from environment variable as the default value. If no value is specified in the environment variable, prompt the user.

        2. For tools that rely on template, the default value of the template is tplv-service_id-image, where service_id needs to be replaced with the actual service_id, e.g., if the user's service_id is n9b2vwdhz3, use tplv-n9b2vwdhz3-image
        ...
        4. `get_image_url_by_store_uri` is used to get the image URL
    ...
    def get_image_url_by_store_uri(
        ...
        URI is obtained from upload_image.
